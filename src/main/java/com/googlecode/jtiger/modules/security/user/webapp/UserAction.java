...
public class UserAction extends ExtJsCrudAction<User, UserManager> {
  ...
  protected Page pageQuery() {
    String hql = "select u from User u inner join u.roles r where r.name=?";
    
    page = getManager().pageQuery(pageOfBlock(),
        hql, UserConstants.ROLE_ADMIN);
    return page;
  }
  ...
  public String resetPassword() {
    user = UserUtil.getPrincipal(getRequest());
    if (user != null && UserConstants.USER_TYPE_SYS.equals(user.getUserType())
        && getModel().getId() != null) {
  ...
  public String updateStatus() {
    if (getModel().getId() != null) {
      getModel()
          .setStatus(
              UserConstants.USER_STATUS_USABLE.equals(getModel().getStatus()) ? UserConstants.USER_STATUS_UNUSABLE
